...
with open('automation_configurations_booking.cfg', 'w') as config_file:
    config.write(config_file)

file_path = download_path + 'tasan booking ' + date_path + '.xls'
...
df.to_excel(download_path+'tasan booking ' + date_path + '.xlsx', index=False)

df = pd.read_excel(download_path + 'tasan booking ' + date_path + '.xlsx')
...
template_file_path = download_path + 'tasan to zoho booking import.xlsx'
...
output_file_path = download_path + 'tasan to zoho booking import ' + date_path + '.xlsx'
...
input("Please make any changes necessary, save on Excel, and exit. Press Enter to continue...")
